...
This script creates
-------------------
- init.sql  (in adversary root)
  > creates DB 'adversary' (DB_DATABASE), DB user, and sets-up needed functionality.

- user.json (in app/utils/jsons/source)
  > an entry for a Adversary Admin is created, so it is usable after build.
...
def adversary_rel_to_abs_path(rel_path):
    adversary_root = os.path.dirname(os.path.realpath(__file__))
    abs_path = os.path.abspath(os.path.join(adversary_root, rel_path))
    return abs_path
...
def main():
    ...
    init_sql_p = adversary_rel_to_abs_path("./init.sql")
    ...
    user_json_p = adversary_rel_to_abs_path("./app/utils/jsons/source/user.json")
