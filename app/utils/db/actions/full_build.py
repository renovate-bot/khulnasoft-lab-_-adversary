...
def main():
    ...
    with app.app_context():
        ...
        if src_mgr.role.load_validate():
            ...
            print("Role is needed for Adversary to function. Exiting.")
            sys.exit(2)
        ...
        if not src_mgr.user.load_validate():
            print("User is needed for Adversary to function. Exiting.")
            sys.exit(3)
        ...
        if len(attack_versions) == 0:
            print("Failed to load any ATT&CK versions. At least one is needed for Adversary to work. Exiting.")
            sys.exit(5)
        else:
